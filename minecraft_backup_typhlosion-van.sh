...
mkdir /home/minecraft/Minecraft_Compress/papernut-typhlosion-van_"$(date +%F)"/
cp -Rv /home/minecraft/papernut/ /home/minecraft/Minecraft_Compress/papernut-typhlosion-van_"$(date +%F)"/
sync
sleep 10

echo "Running 7zip compress command"
/usr/local/bin/7zip /home/minecraft/papernut-typhlosion-van_"$(date +%F)".7z /home/minecraft/Minecraft_Compress/papernut-typhlosion-van_"$(date +%F)"
rm -Rv /home/minecraft/Minecraft_Compress/papernut-typhlosion-van_"$(date +%F)"

echo "Uploading File for the first time..."
cp /home/minecraft/papernut-typhlosion_"$(date +%F)".7z /home/minecraft/GDrive/

main_file=$(stat -c %s /home/minecraft/papernut-typhlosion-van_"$(date +%F)".7z)
remote_file=$(stat -c %s /home/minecraft/GDrive/papernut-typhlosion-van_"$(date +%F)".7z)

main_file_second_check=$(stat -c %s /home/minecraft/papernut-typhlosion-van_"$(date +%F)".7z)
remote_file_second_check=$(stat -c %s /home/minecraft/GDrive/papernut-typhlosion-van_"$(date +%F)".7z)

if [[ "$main_file" -ne "$remote_file" ]]; then
  echo "Files Sizes aren't the same, Trying again..."
  rm /home/minecraft/GDrive/papernut-typhlosion-van_"$(date +%F)".7z
  sleep 5
  sync
  cp /home/minecraft/papernut-typhlosion-van_"$(date +%F)".7z /home/minecraft/GDrive/
  sleep 10
  if [[ "$main_file_second_check" -ne "$remote_file_second_check" ]]; then
    ...
    sync
    sleep 5
    cp -v /home/minecraft/papernut-typhlosion-van_"$(date +%F)".7z /home/minecraft/GDrive/
  fi
else
  echo "File Sizes Are Fine"
fi

md5_main_file=$(md5sum --tag /home/minecraft/papernut-typhlosion-van_"$(date +%F)".7z | cut -d '=' -f 2-)
md5_remote_file=$(md5sum --tag /home/minecraft/GDrive/papernut-typhlosion-van_"$(date +%F)".7z | cut -d '=' -f 2-)

md5_main_file_second_check=$(md5sum --tag /home/minecraft/papernut-typhlosion-van_"$(date +%F)".7z | cut -d '=' -f 2-)
md5_remote_file_second_check=$(md5sum --tag /home/minecraft/GDrive/papernut-typhlosion-van_"$(date +%F)".7z | cut -d '=' -f 2-)

if [ "$md5_main_file" == "$md5_remote_file" ]; then
  echo "File Hashes are fine."
else
    echo "Hashes aren't the same, trying again..."
    rm /home/jesse/GDrive/papernut-typhlosion-van_"$(date +%F)".7z
    sleep 5
    sync
    cp -v /home/minecraft/papernut-typhlosion-van_"$(date +%F)".7z /home/minecraft/GDrive/
    sleep 10
  if [ "$md5_main_file_second_check" == "$md5_remote_file_second_check" ]; then
    echo "File Hashes are fine"
      ...
      fusermount -u /home/minecraft/GDrive/
      rclone mount --daemon GDrive:Minecraft_Backups/ /home/minecraft/GDrive/
      rm /home/jesse/GDrive/Minecraft_Backups/papernut-typhlosion-van_"$(date +%F)".7z
      sync
      sleep 5
      cp -v /home/minecraft/Backups/papernut-typhlosion-van_"$(date +%F)".7z /home/minecraft/GDrive/
  fi
fi

# Start the Server backup
cd /home/minecraft/papernut/
tmux new -d -s vanilla java -Xms8096M -Xmx8096M -XX:+UseG1GC -XX:+UnlockExperimentalVMOptions -XX:MaxGCPauseMillis=100 -XX:+DisableExplicitGC -XX:TargetSurvivorRatio=90 -XX:G1NewSizePercent=50 -XX:G1MaxNewSizePercent=80 -XX:G1MixedGCLiveThresholdPercent=35 -XX:+AlwaysPreTouch -XX:+ParallelRefProcEnabled -Dusing.aikars.flags=mcflags.emc.gs -jar serverjars-1.jar --nogui
echo "Done"
